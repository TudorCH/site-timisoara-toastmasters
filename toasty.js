...
(function () {
  ...
  css.textContent = [
    ...
    '#toasty-win{position:fixed!important;bottom:88px!important;right:16px!important;z-index:2147483646!important;width:calc(100vw - 32px);max-width:320px;background:#fff;border-radius:20px;box-shadow:0 8px 40px rgba(119,36,50,.2);border-top:3px solid #F2DF74;display:none;flex-direction:column;overflow:hidden;font-family:Inter,sans-serif;transition:opacity .25s,transform .25s;}',
  ...
  var iconX   = '<svg id="ti-x" width="22" height="22" fill="none" stroke="#772432" stroke-width="2.5" viewBox="0 0 24 24" style="display:none"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>';
  ...
  win.id = 'toasty-win';
  win.innerHTML = [
    '<div style="background:linear-gradient(135deg,#772432,#5c1a25);padding:14px 16px;display:flex;align-items:center;gap:10px;">',
    ...
    '</div>',
    '<div id="toasty-msgs" aria-live="polite" aria-label="Mesaje Toasty" style="overflow-y:auto;overscroll-behavior:contain;padding:12px;display:flex;flex-direction:column;gap:8px;max-height:220px;min-height:120px;">',
      '<div style="display:flex;gap:8px;align-items:flex-end;">',
        '<div style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#772432,#5c1a25);display:flex;align-items:center;justify-content:center;flex-shrink:0;"><svg width="13" height="13" fill="none" stroke="#F2DF74" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg></div>',
        '<div style="background:#fdf6f0;border-radius:16px 16px 16px 4px;padding:10px 13px;max-width:82%;font-size:13.5px;line-height:1.5;color:#1f2937;">Bună! 👋 Sunt <b>Toasty</b>, asistentul Timișoara Toastmasters.<br>Cu ce te pot ajuta?</div>',
      '</div>',
    '</div>',
    '<div id="toasty-chips" style="padding:0 12px 10px;display:flex;flex-wrap:wrap;gap:6px;">',
      '<button onclick="toastySend(\'Când sunt ședințele?\')" style="all:unset;background:rgba(242,223,116,.12);border:1px solid rgba(242,223,116,.45);border-radius:999px;padding:5px 11px;font-size:12px;color:#772432;font-weight:500;cursor:pointer;">📅 Ședințe</button>',
      '<button onclick="toastySend(\'Cum mă pot înscrie?\')" style="all:unset;background:rgba(242,223,116,.12);border:1px solid rgba(242,223,116,.45);border-radius:999px;padding:5px 11px;font-size:12px;color:#772432;font-weight:500;cursor:pointer;">✍️ Înscriere</button>',
      '<button onclick="toastySend(\'Pot veni gratuit?\')" style="all:unset;background:rgba(242,223,116,.12);border:1px solid rgba(242,223,116,.45);border-radius:999px;padding:5px 11px;font-size:12px;color:#772432;font-weight:500;cursor:pointer;">💰 Cost</button>',
      '<button onclick="toastySend(\'Unde se țin ședințele?\')" style="all:unset;background:rgba(242,223,116,.12);border:1px solid rgba(242,223,116,.45);border-radius:999px;padding:5px 11px;font-size:12px;color:#772432;font-weight:500;cursor:pointer;">📍 Locație</button>',
    '</div>',
    '<div style="padding:10px 12px;border-top:1px solid #f0f0f0;display:flex;gap:8px;align-items:center;">',
      ...
      '<button onclick="toastySendInput()" style="all:unset;width:34px;height:34px;border-radius:50%;background:#772432;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;"><svg width="15" height="15" fill="none" stroke="white" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg></button>',
    '</div>'
  ].join('');
  ...
  /* ── iOS keyboard: repoziționează toasty deasupra tastaturii ── */
  if (window.visualViewport) {
    function onVPResize() {
      var keyboardH = Math.max(0, window.innerHeight - window.visualViewport.height - window.visualViewport.offsetTop);
      var isSm = window.innerWidth >= 640;
      var basBtn = isSm ? 28 : 20;
      var basWin = isSm ? 108 : 88;
      btn.style.setProperty('bottom', (basBtn + keyboardH) + 'px', 'important');
      win.style.setProperty('bottom', (basWin + keyboardH) + 'px', 'important');
    }
